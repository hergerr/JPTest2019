...
public class BoardGame {
    ...
    public static void main(String[] args) {
        ...
        games.stream().filter(g -> g.maxPlayers > 4).filter(g -> g.rating > 8).filter(g -> new BigDecimal(150)
                .compareTo(g.price) > 0).map(g -> g.name.toUpperCase()).forEach(System.out :: println);
